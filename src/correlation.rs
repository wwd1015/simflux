...
impl std::fmt::Display for CorrelationError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            CorrelationError::InvalidMatrix(msg) => write!(f, "Invalid correlation matrix: {}", msg),
...
pub fn validate_correlation_matrix(matrix: &[Vec<f64>]) -> Result<(), CorrelationError> {
    ...
    for row in matrix {
        if row.len() != n {
            return Err(CorrelationError::InvalidMatrix("Matrix is not square".to_string()));
        }
    }
    ...
    for i in 0..n {
        if (matrix[i][i] - 1.0).abs() > 1e-8 {
            return Err(CorrelationError::InvalidMatrix("Diagonal elements must be 1.0".to_string()));
        }
    }
    ...
    for i in 0..n {
        for j in 0..n {
            if (matrix[i][j] - matrix[j][i]).abs() > 1e-8 {
                return Err(CorrelationError::InvalidMatrix("Matrix is not symmetric".to_string()));
            }
        }
    }
    ...
    for i in 0..n {
        for j in 0..n {
            if matrix[i][j].abs() > 1.0 {
                return Err(CorrelationError::InvalidMatrix("Correlation values must be between -1 and 1".to_string()));
            }
        }
    }
...
pub fn generate_correlated_normals(
    ...
    Ok(generate_correlated_normals_batch(n_samples, n_factors, &cholesky, seed))
}
...
impl TwoFactorCorrelationStructure {
    pub fn new(
        ...
        if n_sectors == 0 {
            return Err(CorrelationError::InvalidMatrix("At least one sector is required".to_string()));
        }
        ...
        for &corr in &intra_sector_correlations {
            if !(0.0..=1.0).contains(&corr) {
                return Err(CorrelationError::InvalidMatrix("Intra-sector correlations must be between 0 and 1".to_string()));
            }
        }

        let matrix = sector_correlation_matrix.ok_or_else(|| {
            CorrelationError::InvalidMatrix("sector correlation matrix must be provided".to_string())
        })?;
        ...
        for row in &matrix {
            if row.len() != n_sectors {
                return Err(CorrelationError::InvalidMatrix("sector correlation matrix must be square".to_string()));
            }
        }
    ...
    pub fn generate_factors(&self, n_simulations: usize, seed: Option<u64>) -> Vec<SystematicFactors> {
