...
pub fn calculate_portfolio_loss_statistics(losses: &[f64]) -> PortfolioStatistics {
    ...
    let percentiles = [0.95, 0.99, 0.999];
    let mut percentile_values = Vec::new();
    
    for &p in &percentiles {
        let index = ((p * n) - 1.0).max(0.0) as usize;
        let index = index.min(sorted_losses.len() - 1);
        percentile_values.push(sorted_losses[index]);
    }
    
    PortfolioStatistics {
        mean,
        std_dev,
        var_95: percentile_values[0],
        var_99: percentile_values[1],
        var_999: percentile_values[2],
...
fn calculate_expected_shortfall(sorted_losses: &[f64], confidence_level: f64) -> f64 {
    ...
    let cutoff_index = ((confidence_level * n as f64) as usize).min(n - 1);
    
    if cutoff_index == n - 1 {
        return sorted_losses[cutoff_index];
    }
